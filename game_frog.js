...
const img_frog = document.getElementById("img-frog");
const img_hook  = document.getElementById("img-hook");

const frogRect = new Rectangle(0,_c.height-img_frog.naturalHeight,img_frog.naturalWidth,img_frog.naturalHeight);
const frogMouth = new Point(frogRect.x + frogRect.width * 0.65,frogRect.y + frogRect.height * 0.32);
class GameObject{
...
document.getElementById('start-game').onclick = async function() {
  ...
  for (let i = 0; i < _bananaCount; i++){
    ...
    let banana = new GameObject({
      ...
      img,
      coord,
      boundingBox: new Rectangle(32,34,31,50)
    });
  ...
  let hookOptions = {
    ...
    ctx:_ctx,
    img:null,
    coord:new Point(frogMouth.x + 100,frogMouth.y - 10),
...
function step(){
  ...
  if (!hook.goto){
    ...
    if (_keyPresses["Space"] == "on"){
      ...
      hook.addGotoPoint(frogMouth);
      hook.addGotoPoint(new Point(hook.coord.x, hook.coord.y));
  ...
  if (hook.goto && _grabbedBanana != null){
    ...
    if (_grabbedBanana.coord.distanceTo(frogMouth) < 3 ){
...
function paint(){
  ...
  // draw the frog
  _ctx.drawImage(img_frog,frogRect.x,frogRect.y,frogRect.width,frogRect.height);

  // draw the frog tongue
  let hook = _gameObjects["hook"];
  let pink = "#eb34cf";
  drawCircle(frogMouth,2,pink)
  drawLine(frogMouth,
  hook.coord,pink);
  drawCircle(hook.coord,5,pink);
}
...
function drawCircle(p,r,c){
  _ctx.fillStyle = c;
  _ctx.ellipse(p.x, p.y, r, r,0,0,2 * Math.PI);
  _ctx.fill();
}
